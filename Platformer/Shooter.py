import pygame, os, random, sys, csv
...
GRAVITY = 0.75
ROWS = 16      # Number of rows in the csv file
COLUMNS = 150   # Number of columns in the csv file
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    elif tile >= 9 or tile <= 10:    # Water blocks
...
class Decorations(pygame.sprite.Sprite):
    def __init__(self, img, x, y):
        ...
        self.image = img
        self.rect = self.image.get_rect()
        self.rect.midtop = (x + TILE_SIZE // 2, y + (TILE_TYPE - self.image.get_height()))
...
for rows in range(ROWS):
    r = [-1 ]* COLUMNS
    world_data.append(r)
# Load the level data and create world
with open(f'level{LEVEL}_data.csv', newline = '') as csvfile:
    reader = csv.reader(csvfile, delimiter=',')
    for x, row in enumerate(reader):
        for y, tile in enumerate(row):  # Enumerate keeps a check on which tile we are by columns and rows
